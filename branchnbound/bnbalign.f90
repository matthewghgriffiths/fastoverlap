...
INTEGER, PARAMETER :: PMAXNEI = 60 ! Number of nearest neighbours to store
...
DOUBLE PRECISION W,SINW,COSW,RA,RB,ESTLOWER,ESTUPPER,D,V
...
COSW = COS(W)
!V = SQRT(SUM(VECTOR**2))
!COSP = (V-WIDTH*0.25D0)/SQRT(V**2 - V*WIDTH*0.5 * WIDTH**2/8)
!COSW = MIN(COS(WIDTH*0.5D0), (COS(V)**2 + COSP*SIN(V)**2) * COS(WIDTH*0.5D0) - &
! & (1-COSP)*ABS(SIN(V)*COS(V)*SIN(WIDTH+0.5D)) )
...
!                 & MATMUL(FVECS(:,I),DUMMYDOTDISP(:,:M*NPERM,J1)).GT.0.D0)
                DO J2=1,M*NPERM
                    IF (SUM(FVECS(:,I)*DUMMYDOTDISP(:,I,J1)).GT.0.D0) THEN
                        DUMMYLDISTS2(I,J1) = DUMMYDISTS(I,J1)
                    ELSE
                        DUMMYLDISTS2(I,J1) = DUMMYLDISTS(I,J1)
                    END IF
                END DO
            END DO
