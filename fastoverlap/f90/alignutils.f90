...
!        UNSAFE TO CALL WITHOUT SETTING MODULE VARIABLES
...
DOUBLE PRECISION, SAVE, ALLOCATABLE :: DUMMYDISTS(:,:), DUMMYNEARDISTS(:), PDUMMYND(:)
...
IF (DEBUG) THEN
    ...
    IF (BULKT) THEN
        ...
        WRITE(MYUNIT, '(3F20.10)') DISPBEST(1:3)
    ELSE
...
DO J1=1,NCOORDS
    ...
    DISP(3) = DISP(3) + ZM - BOXLX*NINT(ZM)/BOXLZ
ENDDO
DISP = DISP/NCOORDS
...
DIST = 0.D0
DO J1=1,NCOORDS
    XM = COORDSB(3*J1-2) - COORDSA(3*J1-2) - DISP(1)
    YM = COORDSB(3*J1-1) - COORDSA(3*J1-1) - DISP(2)
    ZM = COORDSB(3*J1  ) - COORDSA(3*J1  ) - DISP(3)
    DIST = DIST + (XM - BOXLX*NINT(XM)/BOXLX)**2 + &
                & (YM - BOXLY*NINT(YM)/BOXLY)**2 + &
                & (ZM - BOXLX*NINT(ZM)/BOXLZ)**2
ENDDO
DIST = SQRT(DIST)
...
CALL PERMPAIRDISTS(COORDSB,COORDSA,NATOMS,PMAXNEI,DUMMYDISTS,DUMMYIDX,NPERMGROUP)
CALL FINDBESTPERM(DUMMYDISTS,DUMMYIDX,NATOMS,PMAXNEI,NEWPERM,DISTANCE,NPERMGROUP,INFO)
...
IF (SIZE(DUMMYDISTS).NE.(PMAXNEI*NATOMS*NPERMGROUP)) THEN
    ...
    ALLOCATE(DUMMYDISTS(PMAXNEI*NATOMS,NPERMGROUP),DUMMYNEARDISTS(NATOMS), &
     ...
     & DINVIDX(NATOMS*NATOMS,NPERMGROUP))
END IF
...
IF (SIZE(LPERM).NE.NATOMS) THEN
    ...
    IF(ALLOCATED(PDUMMYA)) DEALLOCATE(PDUMMYA,PDUMMYB,DUMMYA,DUMMYB,NEWPERM,LPERM, &
     & ALLPERM,SAVEPERM,INVPERMGROUP,DUMMY)
