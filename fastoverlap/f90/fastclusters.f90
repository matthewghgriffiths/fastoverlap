...
! EXTERNAL MODULES
!    COMMONS (commons.f90)
!    ALIGNUTILS depends on LAPACK
!        Module for alignment routines, including a reduced version of MINPERMDIST
!    FASTOVERLAPUTILS (fastutils.f90)
...
USE ALIGNUTILS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, NATOMS, BESTPERM, MYUNIT, &
 & SAVECOORDS, NSTORED, PERMINVOPT, NOINVERSION
...
USE FASTOVERLAPUTILS, ONLY : SETNATOMS
IMPLICIT NONE
...
IF (PERMINVOPT.AND.(.NOT.NOINVERSION)) THEN
...
DOUBLE PRECISION AMPLITUDES(NROTATIONS), BESTDIST, RMATSAVE(3,3), RMAT(3,3), WORSTRAD, DISP(3)
...
DO J=1,NROTATIONS
    ...
    CALL ITERATIVEALIGN(DUMMYB,DUMMYA,NATOMS,DEBUG,0.D0,0.D0,0.D0,.FALSE.,DIST2,DISTANCE,RMAT,DISP,PERMBEST)
