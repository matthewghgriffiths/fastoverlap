...
!         IFFT1D
!***********************************************************************
USE ALIGNUTILS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, NATOMS, BESTPERM, NSETS, SETS, MYUNIT

IMPLICIT NONE

! Variables and arrays needed for peakfinding
INTEGER, PARAMETER :: DEFAULTWIDTH=2
DOUBLE PRECISION, PARAMETER :: DEFAULTTOL=1.D-6
INTEGER, SAVE :: FSIZE, FSHAPE(3)
DOUBLE PRECISION, SAVE, ALLOCATABLE :: FSPACE(:,:,:),FSPACECOPY(:,:,:),GAUSARRAY(:,:,:),FVEC(:),FJAC(:,:)

!! Stuff for permutational alignment
DOUBLE PRECISION, SAVE, ALLOCATABLE :: PDUMMYA(:), PDUMMYB(:), DUMMYA(:), DUMMYB(:), XBESTA(:), XBESTASAVE(:)
...
IF (.NOT.ALL(FSHAPE.EQ.NEWSHAPE)) THEN
    FSHAPE = NEWSHAPE
    IF(ALLOCATED(FSPACE))  DEALLOCATE(FSPACE)
...
SUBROUTINE DEALLOCATEFASTUTILS()
...
IF(ALLOCATED(FVEC))  DEALLOCATE(FVEC)
IF(ALLOCATED(FJAC)) DEALLOCATE(FJAC)
IF(ALLOCATED(FSPACECOPY))  DEALLOCATE(FSPACECOPY)
IF(ALLOCATED(GAUSARRAY)) DEALLOCATE(GAUSARRAY)
...
IF (.NOT.ALL(SHAPE(FSPACECOPY).EQ.FSHAPE)) THEN
    IF(ALLOCATED(FSPACECOPY))  DEALLOCATE(FSPACECOPY)
    IF(ALLOCATED(GAUSARRAY)) DEALLOCATE(GAUSARRAY)
    ALLOCATE(FSPACECOPY(FSHAPE(1),FSHAPE(2),FSHAPE(3)),GAUSARRAY(FSHAPE(1),FSHAPE(2),FSHAPE(3)))
ENDIF
...
DO WHILE(NFOUND.EQ.0)
    DO N=1,NPEAKS
        ...
        IF(INFO.EQ.0.OR.INFO.EQ.5) THEN
            IF (DEBUG) THEN
                WRITE(MYUNIT,'(A)') "fastoverlaputils> WARNING - FINDPEAK failed with error:"
                CALL PRINTLMDERERROR(INFO)
            ENDIF
            EXIT
        ELSE
            IF(INFO.EQ.4.OR.INFO.EQ.6.OR.INFO.EQ.7) THEN
                IF (DEBUG) THEN
                    WRITE(MYUNIT,'(A)') "fastoverlaputils> WARNING - FINDPEAK ended with message"
                    CALL PRINTLMDERERROR(INFO)
                ENDIF
            ENDIF
! Find the location of the peak and subtract this peak from the copy of the data
            NFOUND = NFOUND + 1
    ...
    IF (NFOUND.EQ.0) THEN
        WIDTH = WIDTH + 1
        IF (WIDTH.GT.(MINVAL(FSHAPE)/2)) THEN
            WRITE(MYUNIT,'(A)')  "ERROR fastoverlaputils-FINDPEAKS subroutine failed to find any peaks"
            STOP
        ENDIF
    ENDIF
ENDDO
...
SUBROUTINE ARGSORT(A,A2,ARGS,N)

IMPLICIT NONE

INTEGER, INTENT(IN) :: N
DOUBLE PRECISION, INTENT(IN) :: A(N)
INTEGER, INTENT(OUT) :: ARGS(N)
DOUBLE PRECISION, INTENT(OUT) :: A2(N)
...
DO I=1,N-1
    ...
    IF (IMIN.NE.I) THEN
        ...
        TEMP1 = ARGS(I); ARGS(I) = ARGS(IMIN); ARGS(IMIN) = TEMP1
    END IF
END DO
