...
USE COMMONS, ONLY : PERMGROUP, NPERMSIZE, NPERMGROUP, NATOMS, MYUNIT
...
SUBROUTINE ALIGN1(COORDSB,COORDSA,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,DISTANCE,DIST2,NDISPS,NWAVE,NFSPACE)

USE COMMONS, ONLY: OHCELLT

IMPLICIT NONE

INTEGER, INTENT(IN) :: NATOMS, NDISPS, NFSPACE, NWAVE
LOGICAL, INTENT(IN) :: DEBUG
DOUBLE PRECISION, INTENT(IN) :: BOXLX, BOXLY, BOXLZ
DOUBLE PRECISION, INTENT(INOUT) :: COORDSA(3*NATOMS), COORDSB(3*NATOMS)
DOUBLE PRECISION, INTENT(OUT) :: DISTANCE, DIST2

DOUBLE PRECISION WAVEK(3, 2*NWAVE+1,2*NWAVE+1,2*NWAVE+1), K2

COMPLEX*16 FCOEFFA(NFSPACE,NFSPACE,NFSPACE,NPERMGROUP), FCOEFFB(NFSPACE,NFSPACE,NFSPACE,NPERMGROUP), COEFF
COMPLEX*16 FCOEFFS(NFSPACE,NFSPACE,NFSPACE)
INTEGER JX, JY, JZ
